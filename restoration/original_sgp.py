...
if __name__ == "__main__":
    ...
    for image in sorted(defect_images):
        ...
        for xc, yc in stars:
            ...
            final_params_list.append(
                [image, num_iters, execution_time, star_coord, rel_klds, rel_recon_errors, np.round(flux_before, 3), np.round(flux_after, 3), l1_centroid_err, before_ecc, after_ecc, np.round(before_fwhm.value, 3), np.round(after_fwhm.value, 3), flag]
            )
            count += 1
    ...
    if count == 30:
        ...
        if save:
            ...
            df.columns = ["image", "num_iters", "execution_time", "star_coord", "rel_klds", "rel_recon_errors", "flux_before", "flux_after", "l1_centroid_err", "before_ecc", "after_ecc", "before_fwhm (pix)", "after_fwhm (pix)", "flag"]
